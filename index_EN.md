...
- Problem-solving type
    - Not urgent, but there is a gap between the ideal and reality
    - Examples: lately I can't fall asleep easily, "avoid rocking the boat" culture has spread through the organization and it's losing vitality
    - Tends to be left untouched, and the longer it's left, the harder it becomes to solve
- Potential Risk avoidance type
    - Prevent Trouble or Problem by addressing the seeds in advance
    - Examples: getting a full medical checkup, sleeping 7+ hours every day
    - Defensive problem solving
- Improvement Opportunity type
    - Problem solving that doesn't settle for the status quo and aims for better
    - Examples: training to beat your personal best in a marathon, improving your diet for health, improving routine work or meeting operations
    - Offensive problem solving
- Theme-answering type
    - Problem solving in the form of responding to a given proposition (including self Q&A)
    - Examples: partner: "What should we do for dinner tonight?", self Q&A: "Should my next PC be Windows or Mac?"
    - The theme may be vague, or it may require a meticulous answer
    - 🐰Says this occurs in all of types 1-4
...
- At the individual level, create the best possible solution
    - 🐰There is no absolute correct answer, so it's saying to look for something as good as possible
- At the overall level, rack up wins consistently
    - 🐰It's saying that to enrich both work and life, you want to win consistently, like a 7-3 record
- Keep responding to increasingly advanced problems
    - 🐰As life stages and the times change, problems become more complex, so you should keep honing problem solving to keep up

## Problem Solving Routine

### Overview
- A standard pattern for problem solving. You can also think of it as steps or a process
- Tools:
    - Sky, Rain, Umbrella
...
- Decode: Grasp the true nature of the problem you are facing, including understanding the essence hidden behind it
    - → n problems are defined
- Ideate: For the decoded problems, generate several solution options (choices) to solve them
    - → For each problem, n solutions are proposed
- Evaluate: Rank the generated solution options
    - → The relative merits of each solution are determined
- Choose: Pick the best option
    - → Choose (or have someone choose) a solution based on the ranking
...
- For matrix 1 and matrix 2, plot each option on each
- Create an overall evaluation matrix
- Plot each option on the overall evaluation matrix
    - Decide the vertical axis based on the plot in matrix 1
    - Decide the horizontal axis based on the plot in matrix 2
...
- No overlap AND no omissions

### 🔨2: Polar-opposite options
- Can be used in Ideate
- For a given option (solution), propose its polar opposite
...
- List a perspective together with its paired counterpart
...
- Can be used in Decode and Ideate
- Factorize a problem like an equation
- Makes it easier to discuss quantitatively, such as which factor to increase/decrease and by how much

Examples:

- `Profit = (Price - Cost) * Volume`
    - To increase profit? → You can discuss raising price, lowering cost, increasing volume, etc.
- `Share = Market coverage * Win rate in covered markets`
    - To raise share by 6%? → You can propose variations and discuss how to adjust coverage and win rate to achieve it

### 🔨5: Frameworks
- Can be used in Decode and Ideate
- A thinking framework for grasping the whole picture of what you're examining
- There are many; use as appropriate
- Practically speaking, you can treat it as having no omissions

Example: McKinsey's 7S framework for organizations

- 🐰Too much trouble to summarize, so omitted: [McKinsey's organizational change framework "7S" - Salesforce Blog](https://www.salesforce.com/jp/blog/7s-for-change-organization/)

### 🔨6: That and everything else
- Can be used in Decode, Ideate, and Evaluate
- Tools 1-5 strengthen non-omission, but sometimes they don't fit well → that's where this 6 comes in
